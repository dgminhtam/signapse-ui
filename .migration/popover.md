...
- `components/ui/popover-anchor.tsx:31`: added the app-only `Positioner.anchor` composition for Plate/editor usages that previously depended on Radix `PopoverAnchor`; it uses Base `useRender` for ref composition and the default wrapper remains registry-shaped.
