...
export function getUpdateBlogSchema(dictionary: Dictionary) {
  return z.object({
    ...
    slug: z
      ...
      .optional(),
    content: z.string().min(1, dictionary.blogs.contentRequired).optional(),
    shortDescription: z.string().optional(),
    isVisible: z.boolean().optional(),
  })
}

export type UpdateBlogRequest = z.infer<ReturnType<typeof getUpdateBlogSchema>>
        ...
        render={({ field, fieldState }) => (
          <Field data-invalid={fieldState.invalid}>
            <FieldLabel htmlFor="title">
              {dictionary.blogs.titleLabel} <span className="text-destructive">*</span>
            </FieldLabel>
            <Input
              {...field}
              id="title"
        ...
        render={({ field, fieldState }) => (
          <Field data-invalid={fieldState.invalid}>
            <FieldLabel htmlFor="content">
              {dictionary.blogs.contentLabel} <span className="text-destructive">*</span>
            </FieldLabel>
            <InputGroup>
              <InputGroupTextarea
                {...field}
      ...
      <div className="flex gap-4">
        <Button disabled={form.formState.isSubmitting} type="submit">
          {form.formState.isSubmitting ? (
            <>
              <Spinner data-icon="inline-start" /> {dictionary.blogs.updatePending}
            </>
          ) : (
