...
export const convertSearchCondition = (
  key: string,
  operator: string,
  term: string | string[]
) => {
  ...
  switch (operator) {
    ...
    default:
      safeTerm = Array.isArray(term) ? (term[0] ?? "") : term
      if (
        ["gt", "ge", "lt", "le"].includes(operator) &&
        !isNaN(Number(safeTerm))
      ) {
        return `${key} ${operator} ${safeTerm}`
      }
...
export const buildSortQuery = (
  ...
  const sortArray = (
    typeof sortString === "string" ? sortString : sortString.join(",")
  )
    .split(",")
    .filter(Boolean)
    .map((item) => {
...
export const buildFilterQuery = (searchFields: {
