...
description: Use for Clerk authentication patterns in Next.js App Router projects. Covers auth in Server Components, route protection, proxy or middleware strategies, Server Actions, API routes, and auth-aware caching.
---

# Next.js Patterns

Check `package.json` for the Clerk SDK version before choosing an implementation pattern. This repo currently uses `@clerk/nextjs`, so prefer App Router guidance and server-first auth checks.
