...
The timeline SHALL provide one module-level link to the localized `/events` list. Each event row SHALL be a regular `Item` container with a native button around its visible title only, using the backend event ID, and SHALL NOT expose a per-row canonical `href`. An ordinary pointer or keyboard activation of an available event title button SHALL open the dashboard-owned event quick-detail drawer without changing the current dashboard URL; the rest of the row SHALL remain non-interactive. The quick-detail drawer SHALL provide the explicit action for navigating to the canonical event detail route.
...
- **WHEN** a user activates an available event title button with a pointer click, keyboard Enter, or keyboard Space
...
- **AND** the page transition loading bar does not start for the title activation
