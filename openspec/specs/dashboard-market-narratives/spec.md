# dashboard-market-narratives Specification

## Purpose

Define the live, localized Market Narratives section on the production dashboard using the ranked `marketNarratives` metric returned by `GET /dashboard/summary`.
...
- **WHEN** the user cannot access Graph View or the metric has no available items
