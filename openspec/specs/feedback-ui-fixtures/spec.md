...
## Purpose
Define the deterministic, secret-free HTTP fixture boundary that exercises the same feedback integration path as live backend operation.
...
- **THEN** it verifies left-menu visibility, direct-route gating, explicit pending filter, URL-backed controls, canonical detail navigation, permission/status action combinations, required Promote Issue URL, Dismiss omission, deletion at every status, lifecycle races, screenshot isolation, and preserved queue context through visible behavior
