# shadcn-dialog-close-composition Specification

## Purpose
TBD - created by archiving change standardize-dialog-close-composition. Update Purpose after archive.
## Requirements
### Requirement: Pure close actions use Base DialogClose composition
Dialog footer actions whose only purpose is to close, cancel, or dismiss the dialog SHALL use the migrated shadcn `DialogClose` composition with the existing `Button` through the Base UI `render` contract.
...
- **THEN** the button MUST be composed through the Base UI-backed `DialogClose` wrapper instead of calling `onOpenChange(false)` directly
- **AND** the composition MUST use the official Base UI render contract rather than `asChild`

#### Scenario: Pending close button
- **WHEN** a close-only footer button must be disabled during a pending or loading state
- **THEN** the disabled state MUST remain on the child `Button` rendered through `DialogClose`
...
- **THEN** the implementation MUST keep the controlled `Dialog` root
- **AND** it MUST NOT introduce a trigger solely to match an example structure
...
- **THEN** the footer close or cancel action MUST use the Base UI-backed `DialogClose` composition and rely on the root close callback
...
- **THEN** the implementation MAY keep a local handler
- **AND** the reason MUST be visible from the surrounding code behavior
