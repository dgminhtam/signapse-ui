...
- **AND** `includeAnnotations` is `false` until annotation marker UI is explicitly implemented
...
#### Scenario: Parse annotations without rendering markers
- **WHEN** the backend returns `annotations[]`
- **THEN** the system keeps the annotation payload typed and non-crashing
- **AND** the system does not render chart markers, annotation popups, or annotation panels in this MVP
...
The system SHALL render successful candle responses as a financial candlestick chart using the selected chart engine.
