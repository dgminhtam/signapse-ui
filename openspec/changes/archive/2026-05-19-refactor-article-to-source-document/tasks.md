...
- [x] 4.1 Manual QA transferred out of the agent-owned archive gate.
  User-owned manual QA: Verify canonical source-document flows for list, filter, detail, analyze, delete, and legacy-route retirement.
- [x] 4.2 Manual QA transferred out of the agent-owned archive gate.
  User-owned manual QA: Verify economic-calendar-style source documents render the expected type-specific metadata inside the source-document domain.
- [x] 4.3 Manual QA transferred out of the agent-owned archive gate.
  User-owned manual QA: Verify source create/edit screens can load, save, and display the current backend source schema.
- [ ] 4.4 Run project validation for the affected routes, actions, and retired legacy entry points before closing the change.
  User-owned manual QA: Smoke tests for the affected routes, actions, and retired legacy entry points.
