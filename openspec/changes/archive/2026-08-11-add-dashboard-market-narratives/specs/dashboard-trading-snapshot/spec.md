...
- **AND** the dashboard renders the returned Trading Snapshot, Event Timeline, Assets in Focus, and Market Narratives using the backend-resolved scope and `asOf`
...
- **THEN** the frontend does not render placeholder Trading Snapshot, Event Timeline, Assets in Focus, or Market Narratives values as live data
...
- **WHEN** Trading Snapshot, Event Timeline, Assets in Focus, and Market Narratives are loaded
...
The frontend summary action SHALL use the authenticated transport and SHALL validate the response shape, including `asOf`, `timezone`, `scope`, all seven required metric objects including `recentEvents`, `assetsInFocus`, and `marketNarratives`, metric states `AVAILABLE`, `EMPTY`, `DENIED`, `ERROR`, required nullable metric values, `window`, `statuses`, recent event items, ranked asset items and contexts, ranked Market Narrative items, themes, related assets, and contract-compatible `errorCode` strings before passing data to the UI.

#### Scenario: Valid summary response is accepted

- **WHEN** `GET /dashboard/summary` returns the documented top-level fields, the four Trading Snapshot metrics, `recentEvents`, `assetsInFocus`, and `marketNarratives` with valid metric states and nested fields
...
- **WHEN** a response contains `count: null`, `data: null`, `window: null`, `context.summary: null`, Market Narrative nullable content, nullable asset weight, or `errorCode: null` where allowed by the contract
...
- **WHEN** the response omits one of the seven required metric objects or a required field inside one of those objects
...
The dashboard SHALL render `DENIED` and `ERROR` as explicit localized unavailable/error states for any summary metric, including `recentEvents`, `assetsInFocus`, and `marketNarratives`, and SHALL NOT represent either state as count `0` or a successful empty list. A state or failure in one metric SHALL NOT hide or alter the other metric cards, Event Timeline, Assets in Focus, or Market Narratives in a successful HTTP 200 summary.

#### Scenario: One metric is denied

- **WHEN** one metric, including `recentEvents`, `assetsInFocus`, or `marketNarratives`, has `state = "DENIED"` and a canonical permission `errorCode`
...
- **WHEN** one metric, including `recentEvents`, `assetsInFocus`, or `marketNarratives`, has `state = "ERROR"` and no usable data
- **THEN** that metric shows a localized error state without displaying zero, successful empty content, or stale items
- **AND** retained backend metadata such as an available window, narrative statuses, or other available metrics is not discarded
