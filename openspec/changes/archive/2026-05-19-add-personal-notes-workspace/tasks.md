...
- [x] 7.3 Manual QA transferred out of the agent-owned archive gate.
  User-owned manual QA: Smoke-test permission-gated header visibility, quick Sheet open/close, create, update, sanitized rehydration, full `/notes` reload, pagination URL state, selected-note URL state, presentation mode, dirty discard confirmation, and delete confirmation.
