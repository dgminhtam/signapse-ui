...
- [x] 3.2 Manual QA transferred out of the agent-owned archive gate.
  User-owned manual QA: Smoke-test quick Sheet editing so typing exports HTML without the active-editor runtime error.
- [x] 3.3 Manual QA transferred out of the agent-owned archive gate.
  User-owned manual QA: Smoke-test explicit save, backend sanitized rehydrate, note switching, dirty discard, and empty-note rejection.
- [x] 3.4 Manual QA transferred out of the agent-owned archive gate.
  User-owned manual QA: Smoke-test `/notes` read-only presentation mode to confirm it renders saved HTML without edit/export callbacks.
