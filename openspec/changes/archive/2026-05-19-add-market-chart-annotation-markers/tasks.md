...
- [x] 6.4 Manual QA transferred out of the agent-owned archive gate.
  User-owned manual QA: Smoke test `/market-charts` with annotations disabled, annotations enabled with returned markers, enabled with empty `annotations[]`, and at least one invalid/out-of-range annotation case if a fixture or backend response is available.
