...
The timeline SHALL provide one module-level link to the localized `/events` list and SHALL link each event row to the localized `/events/{id}` detail route using the backend event ID. Links SHALL use the existing locale-aware navigation pattern.
...
- **WHEN** a user activates an available event row
- **THEN** the application navigates to the current-locale `/events/{id}` route for that row's backend ID
