...
The module SHALL render each returned article using its title, description, source name, and publication time when available. Missing optional values SHALL use localized fallbacks. The module SHALL provide a localized header action to the existing `/news-articles` route and SHALL NOT expose internal derivation status, event/calendar relationships, or row-level detail links.
...
- **AND** no row presents a derivation status, event relationship, calendar metadata, or detail link
