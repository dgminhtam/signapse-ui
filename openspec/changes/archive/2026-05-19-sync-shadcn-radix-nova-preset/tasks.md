...
- [x] 5.3 Manual QA transferred out of the agent-owned archive gate.
  User-owned manual QA: Run a browser smoke check on the AI provider model picker dialog to confirm the dialog is crisp, the surrounding page is dimmed, and the footer/header match Nova.
- [x] 5.4 Manual QA transferred out of the agent-owned archive gate.
  User-owned manual QA: Smoke representative shadcn surfaces affected by wrapper sync: select/dropdown, form input group/field, sheet/drawer, sidebar, card/empty, and pagination.
