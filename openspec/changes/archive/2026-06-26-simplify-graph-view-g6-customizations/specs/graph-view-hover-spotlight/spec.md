...
- **THEN** visible graph labels remain readable without making the canvas visually heavy
...
**Reason**: The raw G6 refactor removes custom hover label expansion and keeps hover focused on G6 state emphasis.
**Migration**: Users can read selected node details through the existing active-node inspector.
