...
The assistant SHALL use the promoted persisted conversation surface in a local accessible overlay and SHALL keep open and close state in the protected application shell without changing the current URL or navigation history.
...
- **WHEN** the user activates the close control, presses Escape, or dismisses the overlay through supported focus behavior
