...
### 3. Keep loading and failure subordinate to the chapter copy

The inline capture reserves intrinsic aspect-ratio space to avoid layout shift. A failed inline image exposes localized feedback without removing or contradicting the adjacent chapter explanation. No retry workflow is required for immutable build assets.
...
### 4. Reuse the existing test seams

Server-rendered component tests cover semantic heading order, exact localized copy, catalog-driven presence/absence, and no cross-locale fallback. Existing Playwright landing tests cover actual responsive order and geometry, inline image presence/absence, themes, reduced motion, and axe checks. Tests observe headings, roles, labels, image source/locale, and computed visual order rather than helper names or Tailwind class strings.
