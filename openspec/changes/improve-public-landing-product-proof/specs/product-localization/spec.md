...
The system SHALL source all product-capture labels, captions, alternative text, adjacent Knowledge Graph annotations, and inline failure feedback from matching Vietnamese and English frontend dictionaries. The two dictionaries MUST expose the same product-media message structure while allowing natural localized wording based on each approved capture.
