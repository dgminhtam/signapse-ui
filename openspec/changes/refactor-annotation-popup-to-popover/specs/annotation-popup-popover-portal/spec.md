# annotation-popup-popover-portal Specification

## Purpose

This change is **implementation-only** — it replaces the rendering mechanism of the desktop annotation popup from an absolutely-positioned `<div>` inside the chart container to a Radix `Popover` with portal-based rendering. All spec-level behaviors defined in `market-chart-annotation-popup-surface`, `market-chart-annotation-popup-interaction`, and `market-chart-annotation-markers` remain unchanged.
