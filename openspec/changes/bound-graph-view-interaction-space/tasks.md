...
- [ ] 1.3 Verify panning still works within the allowed range and stops before drifting indefinitely into empty space.
...
- [ ] 2.5 Verify dragging a node inside the bounds remains natural and dragging beyond the bounds stops at the nearest valid point.
...
- [ ] 3.4 Verify the control does not interfere with canvas pan, zoom, or node drag gestures.

## 4. Verification

- [x] 4.1 Run targeted lint for the graph view canvas file.
- [x] 4.2 Run project typecheck.
- [ ] 4.3 Smoke test `/graph-view` with a dense graph payload: pan, zoom, drag inside bounds, drag beyond bounds, and recenter.
