...
The request-access destination MUST be `mailto:request-access@signapse.ai?subject=Signapse%20access%20request`, the localized sign-in destination MUST be `/{lang}/sign-in`, the localized dashboard destination MUST be `/{lang}/dashboard`, and the Hero secondary destination MUST be `#product`.
...
The landing page SHALL organize the product story into the canonical sequence: Public Header, Hero Product Proof, Product Story, Analysis Flow, Trust Boundary, Final Access CTA, and Public Footer.
...
- **THEN** the section appears after the four product chapters and before Trust Boundary
...
- **AND** the page does not render a fake chart, fake metric, fake conversation, fake graph, or fake Telegram message
...
- **AND** Graph View, Live Charts, AI Assistant, and Telegram captures use the approved demo scenario and do not expose private or backend-only state
...
- **AND** the page does not clip feature labels, images, focus states, or navigation controls
