...
**Reason**: Event derivation and content reload are operational controls that do not belong on the reading-first article detail route.

**Migration**: Place original-article access in the provenance row and retain only permitted destructive administration in the compact overflow menu.

### Requirement: Primary derivation action uses concise copy

**Reason**: The primary event derivation action has been removed from the news article detail experience.

**Migration**: Use event-focused or automated workflows for event derivation; provide no replacement action on this route.

### Requirement: Description and article image use balanced summary columns

**Reason**: Equal-height desktop columns create empty space for short summaries and reduce the image to a secondary dashboard panel.

**Migration**: Render summary followed by a wide feature image in the single-column reading flow.
